...
function Panel({ children, style }) {
  return <div style={{ background: THEME.panel, border: `1px solid ${THEME.hairline}`, borderRadius: 6, ...style }}>{children}</div>;
}
    ...
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={data} margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
        ...
        <Tooltip contentStyle={{ background: THEME.panelAlt, border: `1px solid ${THEME.hairline}`, borderRadius: 4, fontSize: 12 }}
          labelStyle={{ color: THEME.inkDim }} itemStyle={{ color: THEME.ink }} />
    ...
    <div style={{ position: "sticky", top: 0, zIndex: 40, background: THEME.navy, borderBottom: `1px solid ${THEME.hairline}` }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 22, padding: "10px 20px" }}>
        <div onClick={() => setPage("markets")} style={{ cursor: "pointer", display: "flex", alignItems: "baseline", gap: 0, flexShrink: 0 }}>
          ...
          <span style={{ color: THEME.gold, fontSize: 42, lineHeight: 0, position: "relative", top: 5, marginLeft: 1 }}>.</span>
        </div>
...
function IndexCard({ idx }) {
  ...
  const periodReturn = demoPeriodReturn(idx.key, period);
  const chartSeries = expanded ? getSeriesForRange(idx.key + "CARD", idx.value, period, customRange, 0.012) : idx.spark;

  return (
    <div className="sd-row-hover" style={{
      cursor: "pointer", border: `1px solid ${expanded ? THEME.gold : THEME.hairline}`, borderRadius: 6, padding: "12px 14px",
      background: THEME.panel, minWidth: expanded ? 264 : 168,
    }}>
      <div onClick={() => setExpanded((e) => !e)} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: THEME.creamDim }}>{idx.name}</div>
      ...
      <div onClick={() => setExpanded((e) => !e)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
        <Move value={expanded ? periodReturn : idx.chgPct} size={12} />
        <Sparkline data={chartSeries} width={expanded ? 190 : 70} height={expanded ? 46 : 24} />
      </div>
      <div onClick={() => setExpanded((e) => !e)} style={{ fontSize: 10, color: THEME.inkDim, marginTop: 4 }}>EOD · {idx.sourceDate}</div>
      {expanded && (
        <div onClick={(e) => e.stopPropagation()} style={{ marginTop: 10, borderTop: `1px solid ${THEME.hairline}`, paddingTop: 10 }}>
          <ReturnRangeSelector active={period} onSelect={setPeriod} customRange={customRange} onCustomRange={setCustomRange} />
          <div style={{ fontSize: 11, color: THEME.inkDim, marginTop: 8 }}>{period} performance (demo): <Move value={periodReturn} size={11.5} /></div>
        </div>
      )}
    </div>
  );
...
function MarketsPage({ mode, setPage, openCompany, watchlist, toggleWatch, compareList, toggleCompare }) {
  ...
  return (
    <div className="sd-fade-in" style={{ padding: "22px 20px 60px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 26 }}>
        ...
        <FeaturedChartCard title="INR / USD" seedKey="USDINRFEATURED" endValue={CURRENCIES[0].rate} live sourceDate={CURRENCIES[0].sourceDate} volatility={0.003} />
      </div>
    </div>
  );
}
...
function ThTooltip({ label, infoKey, sortActive, sortDir, onClick, style, align = "left" }) {
  ...
  return (
    <th
      ...
      onClick={(e) => { onClick(); if (info) setOpen((o) => !o); }}
    ...
    >
      ...
      {open && info && (
        <div onClick={(e) => e.stopPropagation()} className="sd-fade-in" style={{
          position: "absolute", zIndex: 30, top: 26, left: align === "right" ? "auto" : 0, right: align === "right" ? 0 : "auto", width: 230, textAlign: "left", textTransform: "none",
          fontWeight: 400, fontSize: 11.5, lineHeight: 1.45, background: THEME.panelAlt, border: `1px solid ${THEME.hairline}`,
          borderRadius: 5, padding: 10, color: THEME.creamDim, boxShadow: "0 10px 24px rgba(0,0,0,0.4)",
        }}>
...
function CompanyPage({ ticker, mode, watchlist, toggleWatch, compareList, toggleCompare, notes, setNote }) {
  ...
  const ranges = ["1M", "3M", "6M", "1Y", "2Y", "3Y", "5Y", "Max"];
  ...
  return (
    <div className="sd-fade-in" style={{ padding: "22px 20px 70px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      {tab === "Overview" && (
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
          <Panel style={{ padding: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>Performance summary</div>
            <PriceChart series={s.hist["6M"]} height={200} />
          </Panel>
          <Panel style={{ padding: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>Key metrics</div>
            {[["P/E", s.pe], ["P/B", s.pb], ["ROE %", s.roe], ["Div Yield %", s.divYield]].map(([l, v]) => (
              <div key={l} style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, padding: "6px 0", borderBottom: `1px solid ${THEME.hairline}` }}>
                <span style={{ color: THEME.inkDim }}>{l}</span><span className="sd-mono">{v !== null && v !== undefined ? fmtNum(v, 1) : "—"}</span>
              </div>
            ))}
            <div style={{ fontSize: 12, marginTop: 12, padding: 10, borderRadius: 4, background: "rgba(201,162,75,0.06)", border: `1px dashed ${THEME.hairline}` }}>
              <b style={{ color: THEME.gold }}>What changed?</b> {whatChangedBlurb(s.ticker)}
            </div>
      ...
      {tab === "Performance" && (
        <div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 10, alignItems: "center" }}>
            <div style={{ display: "flex", gap: 4 }}>{ranges.map((r) => <Pill key={r} active={range === r} onClick={() => setRange(r)}>{r}</Pill>)}</div>
          ...
          <Panel style={{ padding: 16 }}>
            <PriceChart series={series} benchmarkSeries={benchSeries} benchmarkLabel="Nifty 50 (demo)" height={300} />
          </Panel>
...
function ComparePage({ compareList, toggleCompare, openCompany }) {
  ...
  const metrics = [
    { key: "pe", label: "P/E", fmt: (v) => (v ? fmtNum(v, 1) : "—") },
    { key: "pb", label: "P/B", fmt: (v) => (v ? fmtNum(v, 1) : "—") },
    { key: "roe", label: "ROE %", fmt: (v) => (v !== null ? fmtNum(v, 1) : "—") },
    { key: "roce", label: "ROCE %", fmt: (v) => (v !== null && v !== undefined ? fmtNum(v, 1) : "—") },
    { key: "de", label: "D/E", fmt: (v) => (v !== null && v !== undefined ? fmtNum(v, 2) : "—") },
    { key: "divYield", label: "Div Yield %", fmt: (v) => fmtNum(v, 1) },
    { key: "ret1y", label: "1Y Return %", fmt: (v) => fmtNum(v, 1), isMove: true },
          ...
          <Panel style={{ padding: 16, marginBottom: 16 }}>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart>
                ...
                <Tooltip contentStyle={{ background: THEME.panelAlt, border: `1px solid ${THEME.hairline}`, borderRadius: 4 }} />
...
function CurrencyDetail({ currency, back }) {
  ...
  return (
    <div className="sd-fade-in" style={{ padding: "22px 20px 60px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <SectionHeading eyebrow="Reference rate" title={`INR / ${c.code}`} />
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
        <Panel style={{ padding: 16 }}>
...
function CurrenciesPage() {
  ...
  return (
    <div className="sd-fade-in" style={{ padding: "22px 20px 60px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <p style={{ fontSize: 12.5, color: THEME.inkDim, marginTop: -8, marginBottom: 16 }}>Reference rates, not live tradable FX quotes. Shown for research context, not currency forecasting. Click a currency for its historical view.</p>
        ...
        {CURRENCIES.map((c) => (
          <Panel key={c.code} onClick={() => setActiveCode(c.code)} className="sd-row-hover" style={{ padding: 14, cursor: "pointer" }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <div style={{ fontWeight: 700, fontSize: 13.5 }}>INR / {c.code}</div>
      ...
      <p style={{ fontSize: 11.5, color: THEME.inkDim, marginTop: -8, marginBottom: 12, maxWidth: 780 }}>
        Macro developments affecting the INR relative to the currencies shown above — for understanding market drivers, not forecasting. Click a story to read more.
      </p>
...
export default function StockDekho() {
  ...
  return (
    <div className="sd-root" style={{ background: THEME.navy, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1 }}>
        {page === "markets" && <MarketsPage mode={mode} setPage={setPage} openCompany={openCompany} watchlist={watchlist} toggleWatch={toggleWatch} compareList={compareList} toggleCompare={toggleCompare} />}
